...
if ($method === 'POST') {
    ...
    if ($type === 'master') {
        $stmt = $db->prepare('INSERT INTO pemakaian (no_pemakaian,tanggal,keterangan,id_admin,id_gudang) VALUES (?,?,?,?,?)');
        $stmt->bind_param('sssii', $b['no_pemakaian'], $b['tanggal'], $b['keterangan'], $b['id_admin'], $b['id_gudang']);
        $stmt->execute();
    ...
    if ($type === 'detail') {
        $id_pemakaian = intval($b['id_pemakaian']);
        ...
        $stmt = $db->prepare('INSERT INTO pemakaian_detail (id_pemakaian,id_batch,jumlah) VALUES (?,?,?)');
        $stmt->bind_param('iii', $id_pemakaian, $id_batch, $jumlah);
        $stmt->execute();
        $stmt->close();
